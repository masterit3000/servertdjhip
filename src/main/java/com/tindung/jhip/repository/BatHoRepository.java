...
//    public Optional<BatHo> findOneByCuaHang(Long id, Long cuaHangId);
    ...
    @Query(value = "select b from BatHo b  inner join b.hopdongbh h  where  h.trangthaihopdong = ?1 ")
    ...
    @Query(value = "select sum(b.tienduakhach) from BatHo b inner join b.hopdongbh h inner join h.cuaHang c where c.id =:idcuahang")
