...
public class BatHoResource {
    ...
    public BatHoResource(BatHoService batHoService) {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new batHoDTO, or with status 400 (Bad Request) if the batHo has already an ID
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated batHoDTO,
     * or with status 400 (Bad Request) if the batHoDTO is not valid,
     * or with status 500 (Internal Server Error) if the batHoDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/bat-hos")
    @Timed
     ...
     *
     * @return the ResponseEntity with status 200 (OK) and the list of batHos in body
     */
    @GetMapping("/bat-hos")
    @Timed
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the batHoDTO, or with status 404 (Not Found)
     */
