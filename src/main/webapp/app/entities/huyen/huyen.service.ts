...
export class HuyenService {
    ...
    create(huyen: Huyen): Observable<EntityResponseType> {
        ...
        return this.http
            .post<Huyen>(this.resourceUrl, copy, { observe: 'response' })
    ...
    update(huyen: Huyen): Observable<EntityResponseType> {
        const copy = this.convert(huyen);
        return this.http
            .put<Huyen>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http
            .get<Huyen>(`${this.resourceUrl}/${id}`, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }
    ...
    query(req?: any): Observable<HttpResponse<Huyen[]>> {
        ...
        return this.http
            .get<Huyen[]>(this.resourceUrl, {
                params: options,
                observe: 'response'
            })
            .map((res: HttpResponse<Huyen[]>) =>
