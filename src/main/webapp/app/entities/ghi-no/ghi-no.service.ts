...
export class GhiNoService {
    ...
    create(ghiNo: GhiNo): Observable<EntityResponseType> {
        ...
        return this.http
            .post<GhiNo>(this.resourceUrl, copy, { observe: 'response' })
    ...
    update(ghiNo: GhiNo): Observable<EntityResponseType> {
        const copy = this.convert(ghiNo);
        return this.http
            .put<GhiNo>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http
            .get<GhiNo>(`${this.resourceUrl}/${id}`, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }
    ...
    findByHopDong(id: number): Observable<HttpResponse<GhiNo[]>> {
        return this.http
            .get<GhiNo[]>(`${this.ghinoUrl}/${id}`, { observe: 'response' })
            .map((res: HttpResponse<GhiNo[]>) =>
                this.convertArrayResponse(res)
            );
    }
    ...
    baoCaoNV(
        ...
        return this.http
            .get<GhiNo[]>(
                `${this.baocaoNVUrl}/${loaihopdong}/${startd}/${endd}/${id}`,
                { observe: 'response' }
            )
            .map((res: HttpResponse<GhiNo[]>) =>
                this.convertArrayResponse(res)
    ...
    private convertItemFromServer(ghiNo: GhiNo): GhiNo {
        ...
        copy.ngayghino = this.dateUtils.convertDateTimeFromServer(
            ghiNo.ngayghino
        );
        return copy;
    }
